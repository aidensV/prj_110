...
@can('lkps_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("admin.r_8c_masastudi_lllsn_dok.create") }}">
                Tambah Masa Studi Lulusan Doktor
            </a>
        </div>
    </div>
@endcan
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable">
                ...
                <tbody>
                    @foreach($m_8c_masastudi_lllsn_dok as $key => $m_8c_masastudi_lllsn_dok)
                        <tr data-entry-id="{{ $m_8c_masastudi_lllsn_dok->id }}">
                            <td>
                            ...
                            <td>
                                @can('lkps_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.r_8c_masastudi_lllsn_dok.show', $m_8c_masastudi_lllsn_dok->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                @can('lkps_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.r_8c_masastudi_lllsn_dok.edit', $m_8c_masastudi_lllsn_dok->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
                                @can('lkps_delete')
                                    <form action="{{ route('admin.r_8c_masastudi_lllsn_dok.destroy', $m_8c_masastudi_lllsn_dok->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
<script>
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.r_8c_masastudi_lllsn_dok.massDestroy') }}",
